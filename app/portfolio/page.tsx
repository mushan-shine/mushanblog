  ...
  return (
    <div className="max-w-[960px] mx-auto px-6">
      ...
      <section className="pb-24 flex flex-col gap-3">
        {cases.map((c) => (
          <Link key={c.title} href={c.href} className="group bg-white rounded-xl border border-neutral-200/80 p-4 hover:border-neutral-300 hover:shadow-sm transition-all block">
            <div className="flex items-start justify-between mb-2">
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${c.tagColor}`}>{c.tag}</span>
